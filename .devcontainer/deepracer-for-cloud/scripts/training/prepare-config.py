...
if metrics_prefix is not None:
    # New format: training-metrics/main.json (worker name added by simapp)
    config['METRICS_S3_OBJECT_KEY'] = '{}/training-metrics/main.json'.format(metrics_prefix)
else:
...
if s3_mode == 'profile':
    ...
    s3_profile = None
# Always use training-params/main.yaml format (ignore DR_LOCAL_S3_TRAINING_PARAMS_FILE)
s3_yaml_name = 'training-params/main.yaml'
...
if config['MULTI_CONFIG'] == "True" and num_workers > 1:
    ...
    for i in range(1,num_workers+1,1):
        if i == 1:
            # New format: training-params/main.yaml
            s3_yaml_name_temp = 'training-params/main.yaml'
            ...
            # New format: training-params/sub1.yaml, training-params/sub2.yaml, ...
            s3_yaml_name_temp = 'training-params/sub{}.yaml'.format(i - 1)
