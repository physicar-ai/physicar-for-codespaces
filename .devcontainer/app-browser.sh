#!/bin/bash
# Token-gated tunnel publisher for GitHub Codespaces.
#
...
#   line 2: <label>.physicarcs.com       -- once the tunnel is reachable
#           (the extension derives the raw tunnel hostname from this line
#            as its own fallback)
...
# (_TunnelTokenGate) checks it on tunnel traffic. github.dev is gated by
...
# $2 seconds (default 120). A freshly-minted tunnel hostname is printed to
...
# app.physicar empty. Forcing HTTP/2 (TCP) makes the tunnel reachable within
...
# 3) Bring up the tunnel; once its hostname is actually reachable
...
# joined words) so the tunnel control endpoint (api.*) is never mistaken
# for the tunnel URL.
while true; do
  start_tunnel
  ...
  if wait_reachable "$URL" 120; then
    ...
    # Line 2: physicarcs.com (the Cloudflare Worker rebrand of this tunnel) as
    # the ONLY public URL in the bookmark. The extension derives the raw
    # tunnel hostname from this line by itself and falls back to it when the
    # Worker is down or over quota, so the raw URL is not published here.
    PCS_URL="${URL/.trycloudflare.com/.physicarcs.com}/?token=${TOKEN}"
    write_bookmark "$(printf '%s\n%s' "$GH_URL" "$PCS_URL")"
    echo "[app-browser] line 2 published (physicarcs; raw fallback derived in-extension)"
    break
  fi
